...
$.ajax({
  ...
  data: {
    key: 'WMHBZ-L4IW5-M44IB-I4DXL-IMTDJ-SMFK3',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(106.56256, 29.52168, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function createtime1() {
  var grt = new Date("06/25/2022 10:10:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Ray %c 你正在访问Rayの小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("|「瑞のBlog」") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('「瑞のBlog」的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  var start = new Date("06/25/2002 00:00:00"); // 开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("06/25/2022 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://img.shields.io/badge/Sherryの小屋-上班摸鱼中-6adea8?style=social&logo=cakephp' title='什么时候能够实现财富自由呀~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br>  Y 当前距离 X ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://img.shields.io/badge/Sherryの小屋-打烊休息啦-6adea8?style=social&logo=coffeescript' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> Y 当前距离 X ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://blog-1314261683.cos.ap-chengdu.myqcloud.com/public/day.jpeg);
    --darkmode-bg:url(https://blog-1314261683.cos.ap-chengdu.myqcloud.com/public/night1.jpeg);
    --mobileday-bg: url(https://blog-1314261683.cos.ap-chengdu.myqcloud.com/public/m-day.jpeg);
    --mobilenight-bg: url(https://blog-1314261683.cos.ap-chengdu.myqcloud.com/public/night1.jpeg);
  }`;
}
// 切换背景主函数
function changeBg(s) {
